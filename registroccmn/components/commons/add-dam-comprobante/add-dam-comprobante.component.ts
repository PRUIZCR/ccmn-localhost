...
@Component({
  ...
  providers: [ConfirmationService, MessageService, ValDclRegistroDpmnService, DniService, RucService, BuilderCcmnService, GuiaRemisionService]
})
export class AddDamComprobanteComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.registroCcmnService.ccmn$.subscribe(dataCcmn => {
  ...
  validarDeclaracion(): void {
    ...
    let tipoRegistro = this.registroCcmnService.obtenerTipoRegistro();
    
    if(tipoRegistro === '04' && this.frmCtrlDclRegimen.value !== '10' ){
      this.messageService.clear();
      this.messageService.add({severity:"warn", summary: 'Mensaje',
                detail: 'La Declaración no corresponde a régimen 10 importación para el consumo'});
      return;
    }
    ...
    this.registroCcmnService.datosDeclar = paramBusqDcl;

    // if ( this.registroCcmnService.declaracionEstaRegistrada(paramBusqDcl) ) {
    //   this.messageService.clear();
    //   this.messageService.add({severity:"warn", summary: 'Mensaje',
  ...
  private noSePuedeAddComprob() : boolean {
    let mensajeExisteSerieDeclaracion = '';
    ...
    if(this.frmCtrlTipoComprobante.value === '01')
      mensajeExisteSerieDeclaracion = this.valDclRegDpmService.existeSerieDeclaracion(this.rptaDamSeriesDpmn.data, this.frmCtrlTipoComprobante.value, this.frmCtrlNumGuia.value);
    if (mensajeExisteSerieDeclaracion.trim().length > 0 && this.frmCtrlTipoComprobante.value === '01') {
      this.messageService.clear();
      this.messageService.add({severity:"warn", summary: 'Mensaje',
                detail: mensajeExisteSerieDeclaracion});
      return true;
    }

    let tipoRegistro = this.registroCcmnService.obtenerTipoRegistro();
    let obtenerCodigoAduanaDescarga = this.registroCcmnService.obtenerCodigoAduanaDescarga();
    let codDatacat = this.ubigeoSeleccionado?.codDatacat.substring(0,2);

    if( (tipoRegistro === '04') && 
      (codDatacat === '24' && obtenerCodigoAduanaDescarga === '019' ||  
       codDatacat === '21' && obtenerCodigoAduanaDescarga === '181' ||
       codDatacat === '23' && obtenerCodigoAduanaDescarga === '172'
    )){
      this.messageService.clear();
      this.messageService.add({severity:"warn", summary: 'Mensaje',
            detail: 'El destino no puede ser de la misma aduana de frontera'});
      return true;
    }
  ...
  private agregarComprobanteDeclaracion() : void {
    ...
    this.registroCcmnService.putCcmn(ccmnActualizada);
    this.registroCcmnService.putDamSeriesCcmn(comprobanteNew.numCorrelativo, this.rptaDamSeriesDpmn.data);
  ...
  private iniSubsCtrlsGuiaRemision() : void {
    ...
    this.buscarGuiaRemisionSubs = this.guiaRemisionService.rptaGuiaRemision$.subscribe((rpta : Respuesta<GuiaRemisionTransp>) => {
      ...
      if(rpta.data.respuesta === 'GUIA-MANUAL'){
        noExisteGuiaRemision = false;
      }
  ...
  private esDestinoNoValido() : boolean {
    let mismaCiudad : boolean = this.registroCcmnService.ciudadOrigenIgual(this.ubigeoSeleccionado?.codDatacat);
  ...
  private validarExistenciaGuiaRemision() {
    ...
    let isNumberSerieGuia = Number(numSerieGuia.charAt(0));
    if(isNumberSerieGuia >= 0){
      this.confirmationService.confirm({
        message: 'Se est&aacute; ingresando gu&iacute;a de remisión manual',
  ...
  private mostrarDatosDam(paramBusqDcl : ParamBusqDcl) : void {
    ...
    let tes = this.declaracionService.buscar(codAduana, codRegimen, anio, numero).subscribe((declaracion : Declaracion) => {
      this.rptaDeclaracion = Respuesta.create(declaracion, Estado.SUCCESS);
      let tipoRegistro = this.registroCcmnService.obtenerTipoRegistro();

      if(tipoRegistro === '04' && this.rptaDeclaracion?.data.codModalidad !== '10' && this.rptaDeclaracion?.data.codModalidad !== '01'){
        this.messageService.clear();
        this.messageService.add({severity:"warn", summary: 'Mensaje',
              detail: 'La DAM no pertenece a despacho urgente ni anticipado'});
          
        this.valDclRegDpmService.limpiarDatosBusqDclSource();
        return;
      }

      if(tipoRegistro === '04' && this.rptaDeclaracion?.data.codLugarRecepcion !== '14'){
        this.messageService.clear();
        this.messageService.add({severity:"warn", summary: 'Mensaje',
            detail: 'El punto de llegada no corresponde a un centro de atención fronteriza CAF'});
