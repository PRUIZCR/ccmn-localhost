...
export class BuscarCcmnComponent implements OnInit {
  ...
  private URL_RESOURCE_PUESTO_CONTROL_CCMN: string = environment.urlBase+ConstantesApp.RESOURCE_PUESTO_CONTROL;
  ...
  perfilRegistro: boolean =false ;
  consultarByPerfil: boolean =false ;
  ...
  onBlurValidarRUC(){
    var numRUC = this.consultaRectiCcmnForm.get('numDocIdentidad')?.value;
    this.dniService.buscar(numRUC, "1");
    this.completarDatosConductor(numRUC);
   }
...
cargarPuestoControl(aduanaFuncionario: string) {
  var aduana = this.consultaRectiCcmnForm.get('codAduanaDAM')?.value;
  //var codPuestoControlAdu = this.consultaForm.controls.codPuestoControl.value;
  var codPuestoControlAdu = this.consultaRectiCcmnForm.get('puestoControl')?.value;
  this.http
    .get<any>(this.URL_RESOURCE_PUESTO_CONTROL_CCMN + aduanaFuncionario).subscribe((res: any) => {
      this.upperCase(res);
      ...
      if(busqPuesto==0){
        ...
        this.frmCtrlPuestoControl.setValue(codPuestoControlAdu);
      }

      if(aduana?.value=="019"){
         datCatPtoControl.codigo = "0205";
         datCatPtoControl.descripcion = "0205 - CEBAF EJE VIAL 1";
         this.lstPuestoControl.push(datCatPtoControl);
      }else if(aduana?.value=="181"){
        datCatPtoControl.codigo = "0203";
         datCatPtoControl.descripcion = "0203 - CEBAF DESAGUADERO";
         this.lstPuestoControl.push(datCatPtoControl);
      }else if(aduana?.value=="172"){
        datCatPtoControl.codigo = "0204";
         datCatPtoControl.descripcion = "0204 -CAF SANTA ROSA";
         this.lstPuestoControl.push(datCatPtoControl);
      }
...
upperCase(data:PuestoControl[]){
  data.forEach(function(item:PuestoControl){
    if (item.codigo.trim() != ""){
         item.descripcion=item.codigo+' - '+item.descripcion.toUpperCase();
    }
    return item;
  })
  ...
  consultar() {
    ...
    this.getPerfilesUsuario(this.usuarioLogin, paramConsultar,"CPASO_JEFE_SUPERVISOR", "CPASO_FUNCIONA_REGISTRO");

  }

  public getPerfilesUsuario( usuario: string,paramConsultar:ParamBusqCcmnParaRectificar,perfilSupervisor:string, perfilRegistro:string ) {

    // const boolean = this.perfilesUsuarioService.buscar(usuario,perfilSupervisor );
    // const boolean2 = this.perfilesUsuarioService.buscar(usuario,perfilRegistro )
  this.perfilesUsuarioService.buscar(usuario,perfilSupervisor )
    .subscribe( (data : PerfilUsuario[]) => {
      this.consultarByPerfil=true;
       if (data.length>0) {
      ...
      if(!err){
        this.consultarByPerfil=false;
        this.getPerfilRegistro(usuario,paramConsultar,perfilRegistro);
        // this.messageService.clear();
        // this.messageService.add({ key: 'msj', severity: 'warn', detail: "No existen datos de Perfiles de usuario" });
        //this.loadingConsultar = false;
  ...
  public getPerfilRegistro( usuario: string,paramConsultar:ParamBusqCcmnParaRectificar,perfilRegistro:string ) {

    this.perfilesUsuarioService.buscar(this.usuarioLogin,perfilRegistro ).subscribe( (data2 : PerfilUsuario[]) => {
      if (data2.length>0) {
        paramConsultar.funcionario.tienePerfilJefeSupervisor=false;
  ...
  enabledDAM = () => {
    this.consultaRectiCcmnForm.controls.codAduanaDAM.enable();
